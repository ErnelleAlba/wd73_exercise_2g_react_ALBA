function PokemonCard ({pokemon:{name, type, type_2, hp, attack, defense, sp_atk, sp_def, speed, image_url}}) {

  return (
    <>
      <div className="card m-3 shadow" style={{width: '23rem'}}>
        <div className="bg-light" >
          <img src={`${image_url}`} className="card-img-top" alt={`${name}`} style={{height: '230px', objectFit:'scale-down'}}/>
        </div>
        <div className="card-body">
          ...
          <ul className="list-unstyled ps-3">
            <li className="fw-bold d-flex"><span className="text-secondary me-3">Type:</span>
              <img src={`${type}`} style={{width:'50px'}}/>
              <img className={`${!type_2 ? 'd-none': 'd-block'}`} src={`${type_2}`} style={{width: '50px'}}/>
            </li>
